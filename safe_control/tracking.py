...
class LocalTrackingController:
    def __init__(self, X0, robot_spec,
        ...
        # else: leave self.ax/self.fig as whatever was passed in (None by
        # default) -- no Matplotlib figure/axes are created for headless runs.
    ...
    # # If the 'upper' function is not compatible with device, please use the function provided below
    # def export_video(self):
    #     # convert the image sequence to a video
    #     if self.show_animation and self.save_animation:
    #         subprocess.call(['ffmpeg',
