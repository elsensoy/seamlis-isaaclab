...
# Recent unsafe (visibility-violation) points kept for the scatter-plot
# overlay. The exact violation count is tracked separately in
# self.unsafe_point_count, so bounding this doesn't affect that metric.
UNSAFE_POINTS_MAXLEN = 2000
...
class BaseRobot:
    ...
    def __init__(self, X0, robot_spec, dt, ax=None):
        ...
        ax: plot axis handle, or None for a headless robot with no
            Matplotlib artists (render_plot() becomes a no-op)
        '''
        self.ax = ax
        ...
        # cam_range gates unknown-obstacle detection range for any sensor
        # modality (detect_unknown_obs_fov() reads it unconditionally) -- it
        # used to only get set for sensor == 'rgbd', so a lidar-sensor robot
        # crashed with AttributeError as soon as it had any unknown obstacle
        # to detect (never exercised before since no lidar config had one).
        ...
        if self.robot_spec['model'] == 'SingleIntegrator2D':
            ...
            # Visualization Patches (created below, only if ax is given)
        ...
        # Plot handles -- all Matplotlib artist creation below is gated on
        # `ax is not None`, so headless runs (ax=None) never construct
        # figures/axes/patches. Every attribute still gets a None default so
        # code elsewhere can safely check `is not None` rather than needing
        # hasattr(). render_plot() bails out immediately when self.ax is None,
        # so none of these are ever read in that case.
        self.vis_orient_len = 0.5
        self.vehicle_body = None
        self.front_wheel = None
        self.rear_wheel = None
        self.body_circle = None
        self.orientation_rectangle = None
        self.body_plane = None
        self.front_rect = None
        self.rear_rect = None
        self.thrust_rect = None
        self.elevator_rect = None
        self.max_indicator_width = 1.5    # Maximum length corresponding to full speed
        self.indicator_height = 0.15
        self.velocity_indicator = None
        self.velocity_frame_h = None
        self.velocity_frame_v = None
        self.velocity_text = None
        self.body = None
        self.unsafe_points_handle = None
        self.axis = None
        self.fov = None
        self.fov_fill = None
        self.sensing_footprints_fill = None
        ...
        self.detected_obs_patch = None
        self.detected_points_scatter = None
        ...
        # Store the unsafe points and scatter plot. unsafe_point_count is the
        # exact running total (used as a metric elsewhere); unsafe_points only
        # keeps the most recent points for the scatter-plot overlay.
        self.unsafe_point_count = 0
        self.unsafe_points = deque(maxlen=UNSAFE_POINTS_MAXLEN)
