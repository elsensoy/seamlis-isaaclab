...
# Trail length kept for the trajectory-line visualization. Older points are
# dropped rather than accumulating for the full run (self.trajectory is
# never read for anything besides drawing this trail).
TRAJECTORY_MAXLEN = 5000
...
class DriftingCar:
    ...
    def __init__(self, X0, robot_spec, dt, ax=None):
        ...
        # Trajectory history (bounded -- see TRAJECTORY_MAXLEN)
        self.trajectory = deque([self.X[:2, 0].copy()], maxlen=TRAJECTORY_MAXLEN)
